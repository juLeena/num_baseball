...
select_mode=easygui.buttonbox("모드를 선택하세요.","num_baseball",choices=['1인용','2인용','종료'])
    ...
    if select_mode=='1인용':
        ...
        while 4>len(com):
            ...
            if x not in com:
                com.append(x)
        while True:
            choice_sub=easygui.buttonbox("무엇을 하시겠습니까?","num_baseball",choices=['숫자 찾기','정답 맞추기','새로 시작'])
            if choice_sub=='숫자 찾기':
                strike=0
                ball=0
                result=[]
                check=0
                choice_com=easygui.multenterbox("숫자를 입력하세요.","num_baseball",['첫번째 자리','두번째 자리','세번째 자리','네번째 자리'],['','','',''])
                for i in range(3):
                        for j in range(len(choice_com)-i-1):
                            if choice_com[i]==choice_com[i+j+1]:
                                ...
                                break
                if check==1:
                    easygui.msgbox("같은 숫자는 입력할 수 없습니다. \n각 자리에는 서로 다른 숫자를 입력해주세요" ,"num_baseball")
                else:
                    if len(choice_com[0])==len(choice_com[1])==len(choice_com[2])==len(choice_com[3])==1 :                            
                        ...
                        if ball==0 and strike==0:
                         ...
                         easygui.msgbox("숫자를 잘못 입력했습니다.\n각 자리에는 한 자리 수만 입력해주세요^^","num_baseball")
            elif choice_sub=='정답 맞추기':
                choice_really=easygui.buttonbox("정말 후회 안하시고 정답을 입력할 것 입니까? \n돌이킬 수 없어요. 진심으로.","num_baseball",choices=['정답 맞추기','숫자 찾으러 가기'])
                if choice_really=='정답 맞추기':
                    answer=easygui.multenterbox("컴퓨터의 숫자를 입력하세요.","num_baseball",['첫번째 자리','두번째 자리','세번째 자리','네번째 자리'],['','','',''])
                    if com==answer:
                        correct+=1
                        easygui.msgbox("정답입니다!\n상품을 받아가세요!\n숫자를 찾은 횟수 : "+str(times)+"\n"+"정답 시도 횟수 : "+str(correct),"num_baseball")
                        break
                    else:
                        correct+=1
        ...
        choice_p1=easygui.multenterbox("플레이어 1의 숫자를 입력하세요.","num_baseball",['첫번째 자리','두번째 자리','세번째 자리','네번째 자리'],['','','',''])
        choice_p2=easygui.multenterbox("플레이어 2의 숫자를 입력하세요.","num_baseball",['첫번째 자리','두번째 자리','세번째 자리','네번째 자리'],['','','',''])
        times_p1=0
        times_p2=0
        correct=0
        print('플레이어 1','                ','플레이어 2')
        while True:
            turn_p1=easygui.buttonbox("플레이어 1의 차례입니다.","num_baseball",choices=['숫자 찾기','정답 맞추기'])
            if turn_p1=='숫자 찾기':
                strike=0
                ball=0
                result=[]
                ...
                answer=easygui.multenterbox("플레이어 2의 숫자를 입력하세요.","num_baseball",['첫번째 자리','두번째 자리','세번째 자리','네번째 자리'],['','','',''])
                if choice_p2==answer:
                    correct+=1
                    ...
                    easygui.msgbox("틀렸습니다.\n다시 한 번 잘 생각해보세요.\n숫자를 찾은 횟수 : "+str(times_p1)+"\n"+"정답 시도 횟수 : "+str(correct),"num_baseball")
                    print('오답',end='                        ')
            turn_p2=easygui.buttonbox("플레이어 2의 차례입니다.","num_baseball",choices=['숫자 찾기','정답 맞추기'])
            if turn_p2=='숫자 찾기':
                strike=0
                ball=0
                result=[]
                ...
                answer=easygui.multenterbox("플레이어 1의 숫자를 입력하세요.","num_baseball",['첫번째 자리','두번째 자리','세번째 자리','네번째 자리'],['','','',''])
                if choice_p1==answer:
                    correct+=1
